...
$(function() {
	...
	function doSearch(keyword2, levels) {
		var parms;
		keyword2 = encodeSearchKeyWord(keyword2);
		if (!!levels) {
	...
	AutofillComponent.prototype.getSuggestions = function(keyword2) {
		return $.ajax({
			url: this.serviceUrl,
			dataType: 'jsonp',
			cache: true,
    ...
    $('#keyword2').autocomplete({
		...
		source: function(request, response) {
			...
			var keyword2 = autoFillComponent.term;
			...
			//}
			if (keyword2 !== "") {
				autoFillComponent.getSuggestions(keyword2)
				.done(function(data) {
		...
		open: function() {
            $("#ui-id-1").width($('#keyword2').innerWidth());
		...
		select: function(e, ui) {
			...
			if (ui.item.label !== noResultsMsg) {
				doSearch($('#keyword2').val(), ui.item.category || '');
			} else {
                $('#keyword2').val(autoFillComponent.term);
			}
		}
	}).keypress(function(e) {
		var val = $('#keyword2').val();
		if (e.keyCode === 13) {
			e.preventDefault();
		...
		var srchTerm = $.trim($('#keyword2').val().replace(/[-[\]{}()*+?.,\\^$|#]/g, "\\$&")).split(/\s+/);
	...
	setTimeout(function() {
		$('#keyword2').autocomplete('widget').on('mouseleave', function() {
			$('#keyword2').autocomplete('close');
		});
	}, 200);
});
