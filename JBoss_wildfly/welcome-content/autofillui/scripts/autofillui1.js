...
$(function() {
	...
	function doSearch(keyword1, levels) {
		var parms;
		keyword1 = encodeSearchKeyWord(keyword1);
		if (!!levels) {
	...
	AutofillComponent.prototype.getSuggestions = function(keyword1) {
		return $.ajax({
			url: this.serviceUrl,
			dataType: 'jsonp',
			cache: true,
	...
	var	serviceURL = "http://solrx4119p.stag.ch4.s.com:9190/autofill/search",
    ...
    $('#keyword1').autocomplete({
		...
		source: function(request, response) {
			...
			var keyword1 = autoFillComponent.term;
			...
			//}
			if (keyword1 !== "") {
				autoFillComponent.getSuggestions(keyword1)
				.done(function(data) {
		...
		open: function() {
            $("#ui-id-1").width($('#keyword1').innerWidth());
		...
		select: function(e, ui) {
			...
			if (ui.item.label !== noResultsMsg) {
				doSearch($('#keyword1').val(), ui.item.category || '');
			} else {
                $('#keyword1').val(autoFillComponent.term);
			}
		}
	}).keypress(function(e) {
		...
		var srchTerm = $.trim($('#keyword1').val().replace(/[-[\]{}()*+?.,\\^$|#]/g, "\\$&")).split(/\s+/);
	...
	setTimeout(function() {
		$('#keyword1').autocomplete('widget').on('mouseleave', function() {
			$('#keyword1').autocomplete('close');
		});
	}, 200);
});
